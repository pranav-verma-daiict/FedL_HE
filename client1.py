...
# Train the model on client1's data
def train():
    model.train()
...
if __name__ == "__main__":
    ...
    requests.post('http://127.0.0.1:5000/context', data=context.serialize())
    ...
    while True:
        ...
        print(f"Client1 - Epoch {epoch}: Accuracy before update: {accuracy_before:.2f}%")

        params_array = get_params_as_array(model)

        # Encrypt parameters
        enc_params = ts.ckks_vector(context, params_array)
        response = requests.post('http://127.0.0.1:5000/update', data=pickle.dumps(enc_params.serialize()))
        
        # Print the response content for debugging
        print("Server response content:", response.content)
        
        updated_enc_params = ts.ckks_vector_from(context, pickle.loads(response.content))

        # Decrypt updated parameters
        updated_params_array = np.array(updated_enc_params.decrypt(), dtype=np.float32)  # Ensure float32
        ...
        print(f"Client1 - Epoch {epoch}: Accuracy after update: {accuracy_after:.2f}%")

        if accuracy_after >= 80.0:
            print(f"Client1 achieved an accuracy of {accuracy_after:.2f}% in epoch {epoch}. Stopping training.")
            break
